...
A concept of dynamic-link libraries shows its potential when applied on libraries used by multiple applications. Dynamic-link libraries are also called shared libraries. These are the advantages of using shared libraries:
...
After loading the library into the computer memory, a specific memory address of the accessed function must be obtained. Owing to the compilation time function export, the WinAPI or Qt library can get the exact address of the function. A \clist{GetProcAddress} function from WinAPI or \clist{QLibrary::resolve} function from Qt library can be used (line \ref{lst:getproc} on Listing \ref{runtime}).
